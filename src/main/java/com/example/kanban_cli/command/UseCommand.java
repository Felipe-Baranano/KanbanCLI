...
@Command(name = "use", description = "Select a collection to work with")
public class UseCommand implements Runnable {
    
    @Parameters(index = "0", arity = "1..*", description = "Collection name")
    ...
    public void run() {
        try {
            ...
            if (collection == null) {
                ...
                collectionDAO.updateIsActive(true, collectionName);
